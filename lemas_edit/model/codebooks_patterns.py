...
class Pattern:
    ...
    def build_pattern_sequence(self, z: torch.Tensor, special_token: int, keep_only_valid_steps: bool = False):
        ...
        """
        B, K, T = z.shape
        # Guard against degenerate zero-length sequences. This can happen
        # in rare edge cases where a segment is empty; in that case we just
        # return a single-step sequence filled with the special token.
        if T == 0:
            device = z.device
            indexes = torch.zeros(K, 1, dtype=torch.long, device=device)
            mask = torch.zeros(K, 1, dtype=torch.bool, device=device)
            values = torch.full((B, K, 1), special_token, dtype=z.dtype, device=device)
